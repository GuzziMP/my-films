    ...
    public string Info
    {
      get
      {
        if (StartCounter > 0)
        {
          ...
          return string.Format("Watch: Started: {1} time(s) - Elapsed: {2}ms ({3} µs) -> {4} ms ({5} µs)/start \"{0}\"", Name, StartCounter, ElapsedMilliseconds, ElapsedMicroseconds, msStart, usStart);
        }
